...
# The Simulator class contains all function that is directly called by the
# simulator.  For example, initilization of the system, and operations in each
# time round.
...
class Simulator:
    def __init__(self, scenario, engine, performance):
        ...
        # mapping from order type to order sets. The value element is a set
        # containing all orders of a particular type.
        ...
        # mapping from peer type to peer sets. The value element is a set
        # containing all peers of a particular type.
    ...
    # Construct a number of peers and a number of orders and maintain their
    # references in two sets.
    # Sequence numbers of peers and neighbors begin from 0 and increase by 1
    # each time.
    # Right now there is no use for the sequence numbers but we keep them for
    # potential future use.
    # We only consider one type of orders for now. However, we do consider
    # multiple peer types.
    """
    ...
    def globalInit(self):
        ...
        )  # order sequence number should start from 0, but can be customized
        ...
        for peer_type in peer_type_vector:
            ...
            # Randomized over [0, birth_time_span] to avoid sequentiality
            # issue.
            ...
            for _ in range(num_orders):
                ...
                # create the order. Order's birth time is cur_time, different
                # from peer's birthtime.
                # Order's creator is set to be None since the peer is not
                # initiated, but will be changed in the peer's initiation
                # function.
                new_order = Order(
        ...
        # update the latest order sequence number and latest peer sequence
        # number
        self.latest_order_seq = order_seq
        self.latest_peer_seq = peer_seq

        # add neighbors to the peers. Use shuffle function to avoid preference
        # of forming neighbors for peers with small sequence number.
        peer_list = list(self.peer_full_set)
        random.shuffle(peer_list)
        self.checkAddingNeighbor()

    # when a new peer arrives, it may already have a set of orders. It only
    # needs to specify the number of initial orders, and the function will
    # specify the sequence numbers for the peers and orders.
    """
    In current implementation we assume there is only one order type.
    There is a hard-coded line of creating orders of type 'default',
    ...
    def peerArrival(self, peer_type, num_orders):
        ...
        for _ in range(num_orders):
            ...
            # Now we initiate the new orders, whose creator should be the new
            # peer.
            # But the new peer has not been initiated, so we set the creator to
            # be None temporarily.
            # We will modify it when the peer is initiated.
            # This is tricky and informal, but I don't have a better way of
            # doing it right now.
            new_order = Order(
                self.scenario, order_seq, self.cur_time, None, expiration
            )
    ...
    # This function initiates an external order arrival, whose creator is the
    # "target_peer"
    ...
    # This function takes a set of orders to depart as input, deletes them, and
    # deletes all other invalid orders from both order set of Simulator, and
    # all peers' pending tables and storages.
    ...
    # The following function helps the requester peer to add neighbors, and is
    # called only by checkAddingNeighbor().
    # It targets at adding demand neighbors, but is fine if the final added
    # number is in the range [mininum, demand], or stops when all possible
    # links are added.
    # This function will call the corresponding peers' functions to add the
            ...
            for candidate in selected_peer_set:
                ...
                if (
                    ...
                    if not candidate.addNeighbor(
                        ...
                        raise RuntimeError(
                            "Function addNewLinksHelper tries to add some"
                            " existing neighbor again."
                        )
                    links_added += 1
    ...
    # This is a temparaty version. In the next PR, mode will not exist. Please
    # ignore the missing of
    ...
    def operationsInATimeRound(
        ...
        # Now, if the system does not have any peers, stop operations in this
        # round.
        # The simulator can still run, hoping that in the next round peers will
        # appear.

        if not self.peer_full_set:
            return

        # if there are only free-riders, then there will be no new order
        # arrival.
        ...
        # The probablity for any peer to get an order is proportional to its
        # init orderbook size.
        # Free riders will not be candidates since they don't have init
        # orderbook.
        ...
        for target_peer in target_peer_list:
            # decide the max expiration for this order. Assuming there is only
            # one type of orders 'default'. Subject to change later.
            expiration = max(
                0,
                round(
                    random.gauss(
    ...
    # This is the function that runs the simulator, including the
    # initilization, and growth and stable periods.
    ...
    def run(self):
        ...
        # for each round of simulation, clear everything:
        self.peer_full_set.clear()
        ...
        # this is very important since numpy.random is not multiprocessing safe
        # (when we call Hawkes process)
        numpy.random.seed()
        counts_growth = list(
            map(
        ...
        # input arguments are: time, peer set, normal peer set, free rider set,
        # order set
