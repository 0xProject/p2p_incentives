...
- There are four ways to receive orders: initialization, addNeighbor, receiveOrderExternal, receiveOrderInternal
    ...
    - 3.1. In Initialization, initial orders are directly put into the local storage (not through pending list).
    ...
    - 3.3. For receiveOrderExternal, it is called when a new order arrives (in the orderArrival function).
            Inside this function, a peer will decide whether to store this newly arrived order into its local storage
            without going through the pending list.
    ...
    - There are some manually different treatments between internal and external order receipt. They are on purpose, and will be
        explained in order sharing section.
      
4. Order storing:

- Among the four ways that a peer can receive an order, 3.2 and 3.4 will first store an order into the pending list. In each round of the
...
- For 3.1 and 3.3, orders at the initialization will be directly stored in the local storage, so no function needs calling.

5. Order sharing:

- A peer will share its orders to only (1) existing neighbors, and (2) new neighbors.
     - For existing neighbors, a peer only needs to share its newly stored orders
       (not existing ones since they should have been shared if deemed).
         - If a new order is stored via internal sharing, it must have been processed
             by storeOrderDecision() so as to be moved from pending list to local storage.
             Inside the storeOrderDecision() function, shareOrderDecision() is called.
         - If a new order is stored via external arrival, then inside the receiveOrderExternal()
             function, shareOrderDecision() is called.
     - For new neighbors (including the init of a new peer), all local orders 
         need to be considered sharing. Inside addNeighbor(), shareNewNeighborDecision() is called.
     - To summarize, all orders are considered being shared.
    
- Recall that there are different treatment between internal and external order arrival. Below is the reason.
    - For internal order arrival, it is the result of a previous order propagation. We first record it in pending table,
        then probably move to local storage, and then decide to further propagate it to other peers by putting it into
        other peers pending list. This is done at the end of this time round so the other peers will not do anything right now,
        but in the next time round. So the effect of further propagation will be shown in the next time round,
        when other peers run storeOrderDecision().
    - For external order arrival (and new peer arrival with init orders), it is NOT the result of a propagation, and we
        want it to be propagated right now (but not in the next round). So we directly decide whether to store it, and put
        it into neighbor's pending list, so that in this round, when peers run storeOrderDecision(), this order will probably be
        stored in neighbor's storage as well, so it is propagated.
...
ROUND = 50 # total round of simulations
...
class Peer:
    ...
    # OrderInfo will be directly put into the local storage, and share decision to neighbors will be directly made.
    # This is for sake that this order can be propagated this round.
    ...
    def receiveOrderExternal(self, order_id):
        ...
        if storeExtHelper(order_instance) is True:
            ...
            # update the number of replicas for this order and holders of this order
            global_id_order_mapping_table[order_id].num_replicas += 1
            global_id_order_mapping_table[order_id].holders.add(self.id)
            
            # decide whether to share it to my neighbors
            self.shareOrderDecision(new_orderinfo)
