...
class Peer:
    ...
    def __init__(
        ...
        self.seq = (
            seq
        )  # sequence number. Not in use now, for reserve purpose only.
        self.birthtime = birthtime
        self.init_orderbook_size = len(init_orders)
        self.namespacing = (
            namespacing
        )  # A peer's namespacing is its interest in certain trading groups. Currently we don't set it.

        self.peer_type = (
            peer_type
        )  # Refers to a peer type (e.g., big/small relayer). Its value is a string (or None by default).
        ...
        for (
            ...
            init_orders
        ):  # inital orders will directly be stored without going through the storage decison.
            ...
            new_orderinfo.storage_decision = (
                True
    ...
    def delNeighbor(self, peer, remove_order=False, notification=True):
        ...
        if remove_order:
            ...
            for (
                order,
                orderinfolist,
            ) in self.order_pending_orderinfo_mapping.items():
                for idx, orderinfo in enumerate(orderinfolist):
                    if orderinfo.prev_owner == peer:
                        del orderinfolist[idx]
                if (
                    orderinfolist == []
                ):  # no pending orderinfo for this order. need to delete this entry
    ...
    def receiveOrderExternal(self, order):
        ...
        if self.engine.externalOrderAcceptance(self, order):
            ...
            new_orderinfo = OrderInfo(
                self.engine, order, self, self.local_clock
            )
    ...
    def receiveOrderInternal(self, peer, order, novelty_update=False):
        ...
        if (
            ...
            raise ValueError(
                "Order transmission cannot be peformed between non-neighbors."
            )
        ...
        if (
            order not in self.order_pending_orderinfo_mapping
        ):  # order id not in the pending set
            self.order_pending_orderinfo_mapping[order] = [new_orderinfo]
    ...
    def storeOrders(self):
        ...
        for (
            ...
            pending_orderinfolist_for_same_order.sort(
            ...
            if not pending_orderinfolist_for_same_order[
                0
            ].storage_decision:  # if nothing is to be stored
                for pending_orderinfo in pending_orderinfolist_for_same_order:
                    ...
                    if (
                        ...
                        in self.peer_neighbor_mapping
                    ):  # internal order, sender is still a neighbor
                        self.peer_neighbor_mapping[
                            pending_orderinfo.prev_owner
                        ].share_contribution[-1] += self.engine.rc

            else:  # the first element is to be stored
                first_pending_orderinfo = pending_orderinfolist_for_same_order[
                    0
                ]
                ...
                if (
                    ...
                    self.peer_neighbor_mapping[
                        first_pending_orderinfo.prev_owner
                    ].share_contribution[-1] += self.engine.rd
                ...
                for pending_orderinfo in pending_orderinfolist_for_same_order[
                    ...
                    if pending_orderinfo.storage_decision:
                        raise ValueError(
                            "Should not store multiple orders. Wrong in order store decision process."
                        )
                    ...
                    if (
                        ...
                        self.peer_neighbor_mapping[
                            pending_orderinfo.prev_owner
                        ].share_contribution[-1] += self.engine.re
    ...
    def shareOrders(self):
        ...
        # peers to share
        peer_to_share_set = self.engine.neighborsToShare(
    ...
    def rankNeighbors(self):
        ...
        peer_list.sort(
            key=lambda item: self.peer_neighbor_mapping[item].score,
            reverse=True,
        )
        return peer_list
