...
class Order:
    def __init__(
        ...
        self.scenario = (
            scenario
        )  # assumption. needed for updateSettledStatus().
        self.seq = (
            seq
        )  # sequence number. Not in use now, reserved for possible future use
        self.birthtime = birthtime  # will be decided by system clock
        self.creator = creator  # the peer who creates this order
        self.expiration = (
            expiration
        )  # maximum time for a peer to be valid, and will expire thereafter
        self.category = (
            category
        ...
        self.is_settled = (
            False
        )  # this order instance has not been taken and settled
        self.is_canceled = (
            False
        )  # will change to True when the order departs proactively.
...
class OrderInfo:
    def __init__(
        ...
        self.engine = (
            engine
        )  # design choice. Needed for orderinfoSetPriority()
        self.arrival_time = (
            arrival_time
        )  # arrival time of this orderinfo to my pending table
        self.prev_owner = (
            prev_owner
        )  # previous owner, default is None (a new order)
        self.novelty = (
            novelty
        )  # How many hops it has travalled. Default is 0. Leave design space for possible extensions in future.
        self.engine.orderinfoSetPriority(
            master, order, priority
        )  # set up priority
